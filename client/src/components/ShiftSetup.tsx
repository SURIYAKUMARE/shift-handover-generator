...
export const ShiftSetup: React.FC<ShiftSetupProps> = ({
  ...
  const getSourceIcon = (id: string) => {
    switch (id) {
      ...
      case 'incident':
        return <Radio className="w-4 h-4 text-[#F59E0B]" />;
      case 'chat':
        return <MessageSquare className="w-4 h-4 text-[#10B981]" />;
      default:
        return <Cpu className="w-4 h-4 text-[#94A3B8]" />;
    }
  };
  ...
  return (
    <div className="bg-[#12171F] border border-[#1E2633] rounded-xl p-5 sm:p-6 mb-6 shadow-console">
      {/* Title & Quick Presets */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-[#1E2633] gap-3">
        <div className="flex items-center gap-2.5">
          <Calendar className="w-5 h-5 text-[#3B82F6]" />
          <h2 className="text-sm sm:text-base font-bold text-[#F8FAFC] uppercase tracking-wide">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm font-sans">
          <span className="text-[#64748B] font-semibold">Quick shifts:</span>
          <button
            type="button"
            onClick={() => setShiftInterval('00:00', '08:00')}
            className="px-3 py-1 rounded-md bg-[#0A0D12] border border-[#283446] text-[#94A3B8] hover:text-[#F8FAFC] hover:border-[#3B82F6] font-medium transition-colors"
          >
            Night (00:00-08:00)
          </button>
          <button
      ...
      {/* Inputs Row (Larger & Prominent) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
        {/* Shift Start */}
        <div>
          <label className="block text-xs sm:text-sm font-sans text-[#94A3B8] mb-1.5 font-semibold">
          ...
          <input
            ...
            className="w-full bg-[#0A0D12] border border-[#283446] rounded-md px-3.5 py-2.5 text-xs sm:text-sm font-mono text-[#F8FAFC] focus:outline-none focus:border-[#3B82F6] transition-colors"
        ...
        {/* Timezone */}
        <div>
          <label className="block text-xs sm:text-sm font-sans text-[#94A3B8] mb-1.5 font-semibold flex items-center justify-between">
      ...
      <div className="mt-5 pt-4 border-t border-[#1E2633]">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs sm:text-sm font-sans text-[#94A3B8] font-semibold">
            Connected Telemetry Feeds
          </span>
          <button
            onClick={onRefreshHealth}
            className="text-xs sm:text-sm text-[#94A3B8] hover:text-[#F8FAFC] flex items-center gap-1.5 font-mono transition-colors"
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between p-3.5 rounded-lg border cursor-pointer transition-all ${
                  isEnabled
                    ? 'bg-[#0A0D12] border-[#283446] hover:border-[#3B82F6]'
                    : 'bg-[#0A0D12]/40 border-[#1E2633] opacity-40'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-md bg-[#18202C]">
                    {getSourceIcon(src.id)}
                  </div>
                  <div>
                    <span className="text-sm font-bold text-[#F8FAFC] block">{src.name}</span>
                    <span className="text-xs text-[#64748B] font-mono block">{src.desc}</span>
                  </div>
                </div>

                <div>
                  {isUnreachable ? (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-mono font-bold text-[#F59E0B] bg-[#F59E0B]/10 border border-[#F59E0B]/30">
                      <span className="w-2 h-2 rounded-full bg-[#F59E0B]" />
                      UNREACHABLE
                    </span>
                  ) : health?.status === 'connected' ? (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-mono font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/30">
                      <span className="w-2 h-2 rounded-full bg-emerald-400" />
                      {health.latency_ms}ms
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-mono text-[#94A3B8] bg-[#18202C] border border-[#283446]">
                      READY
                    </span>
                  )}
                </div>
        ...
        {/* Pre-Generation Carry-Forward Preview Banner */}
        {carriedOverItems && carriedOverItems.length > 0 && (
          <div className="mt-4 p-3.5 sm:p-4 rounded-lg bg-[#0A0D12] border border-[#3B82F6]/40 flex flex-wrap items-center justify-between gap-3 text-xs sm:text-sm">
            <div className="flex items-center gap-2.5">
              <ArrowRightLeft className="w-4 h-4 text-[#3B82F6] shrink-0" />
              <span className="text-[#F8FAFC] font-semibold font-sans">
            ...
            <span className="text-xs font-mono font-medium text-[#3B82F6] bg-[#12171F] px-3 py-1 rounded-md border border-[#283446]">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2.5 text-xs sm:text-sm">
          <span className="font-sans font-semibold text-[#64748B]">Fault simulation tests:</span>
          <button
            type="button"
            onClick={() => setSimulateUnreachable(simulateUnreachable === 'chat' ? '' : 'chat')}
            className={`px-3 py-1 rounded-md text-xs sm:text-sm font-mono border transition-colors ${
